...
class HYRequest {
  ...
  constructor(config: HYRequestConfig) {
    ...
    this.instance.interceptors.response.use(
      this.interceptors?.resonpseInterceptor,
      this.interceptors?.resonpseInterceptorCatch
    )
  ...
  request<T>(config: HYRequestConfig<T>): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      this.instance
        ...
        .then((res) => {
          // 1.单个请求对数据的处理
          if (config.interceptors?.resonpseInterceptor) {
            res = config.interceptors.resonpseInterceptor(res)
          }
